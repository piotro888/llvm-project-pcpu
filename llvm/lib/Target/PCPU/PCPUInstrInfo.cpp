...
void PCPUInstrInfo::copyPhysReg(MachineBasicBlock &MBB,
  ...
  BuildMI(MBB, Position, DL, get(PCPU::OR_I_LO), DestinationRegister)
...
void PCPUInstrInfo::storeRegToStackSlot(
  ...
  BuildMI(MBB, Position, DL, get(PCPU::SW_RI))
...
void PCPUInstrInfo::loadRegFromStackSlot(
  ...
  BuildMI(MBB, Position, DL, get(PCPU::LDW_RI), DestinationRegister)
      .addFrameIndex(FrameIndex)
      .addImm(0)
      .addImm(LPAC::ADD);
}

bool PCPUInstrInfo::areMemAccessesTriviallyDisjoint(
...
PCPUInstrInfo::decomposeMachineOperandsTargetFlags(unsigned TF) const {
  return std::make_pair(TF, 0u);
}

ArrayRef<std::pair<unsigned, const char *>>
PCPUInstrInfo::getSerializableDirectMachineOperandTargetFlags() const {
  using namespace PCPUII;
...
bool PCPUInstrInfo::analyzeCompare(const MachineInstr &MI, Register &SrcReg,
                                    Register &SrcReg2, int64_t &CmpMask,
                                    int64_t &CmpValue) const {
  switch (MI.getOpcode()) {
  default:
    break;
  case PCPU::SFSUB_F_RI_LO:
  case PCPU::SFSUB_F_RI_HI:
    SrcReg = MI.getOperand(0).getReg();
    SrcReg2 = Register();
    CmpMask = ~0;
    CmpValue = MI.getOperand(1).getImm();
    return true;
  case PCPU::SFSUB_F_RR:
    SrcReg = MI.getOperand(0).getReg();
...
inline static bool isRedundantFlagInstr(MachineInstr *CmpI, unsigned SrcReg,
                                        ...
                                        MachineInstr *OI) {
  if (CmpI->getOpcode() == PCPU::SFSUB_F_RR &&
      OI->getOpcode() == PCPU::SUB_R &&
  ...
  if (((CmpI->getOpcode() == PCPU::SFSUB_F_RI_LO &&
        OI->getOpcode() == PCPU::SUB_I_LO) ||
       (CmpI->getOpcode() == PCPU::SFSUB_F_RI_HI &&
        OI->getOpcode() == PCPU::SUB_I_HI)) &&
      OI->getOperand(1).getReg() == SrcReg &&
...
inline static unsigned flagSettingOpcodeVariant(unsigned OldOpcode) {
  switch (OldOpcode) {
  case PCPU::ADD_I_HI:
    return PCPU::ADD_F_I_HI;
  case PCPU::ADD_I_LO:
    return PCPU::ADD_F_I_LO;
  case PCPU::ADD_R:
    return PCPU::ADD_F_R;
  case PCPU::ADDC_I_HI:
    return PCPU::ADDC_F_I_HI;
  case PCPU::ADDC_I_LO:
    return PCPU::ADDC_F_I_LO;
  case PCPU::ADDC_R:
    return PCPU::ADDC_F_R;
  case PCPU::AND_I_HI:
    return PCPU::AND_F_I_HI;
  case PCPU::AND_I_LO:
    return PCPU::AND_F_I_LO;
  case PCPU::AND_R:
    return PCPU::AND_F_R;
  case PCPU::OR_I_HI:
    return PCPU::OR_F_I_HI;
  case PCPU::OR_I_LO:
    return PCPU::OR_F_I_LO;
  case PCPU::OR_R:
    return PCPU::OR_F_R;
  case PCPU::SL_I:
    return PCPU::SL_F_I;
  ...
  case PCPU::SRA_R:
    return PCPU::SRA_F_R;
  case PCPU::SUB_I_HI:
    return PCPU::SUB_F_I_HI;
  case PCPU::SUB_I_LO:
    return PCPU::SUB_F_I_LO;
  case PCPU::SUB_R:
    return PCPU::SUB_F_R;
  case PCPU::SUBB_I_HI:
    return PCPU::SUBB_F_I_HI;
  case PCPU::SUBB_I_LO:
    return PCPU::SUBB_F_I_LO;
  case PCPU::SUBB_R:
    return PCPU::SUBB_F_R;
  case PCPU::XOR_I_HI:
    return PCPU::XOR_F_I_HI;
  case PCPU::XOR_I_LO:
    return PCPU::XOR_F_I_LO;
  case PCPU::XOR_R:
    return PCPU::XOR_F_R;
  default:
    return PCPU::NOP;
...
bool PCPUInstrInfo::optimizeCompareInstr(
  ...
  if (SrcReg2 != 0)
    ...
    if (CmpInstr.getOpcode() == PCPU::SFSUB_F_RI_LO)
      MI = nullptr;
    else
      return false;
  }
  ...
  for (--I; I != E; --I) {
    ...
    if (Instr.modifiesRegister(PCPU::SR, TRI) ||
        Instr.readsRegister(PCPU::SR, TRI))
  ...
  if (flagSettingOpcodeVariant(MI->getOpcode()) != PCPU::NOP) {
    bool isSafe = false;
    ...
    while (!isSafe && ++I != E) {
      ...
      for (unsigned IO = 0, EO = Instr.getNumOperands(); !isSafe && IO != EO;
        ...
        if (MO.isRegMask() && MO.clobbersPhysReg(PCPU::SR)) {
          isSafe = true;
          break;
        }
        if (!MO.isReg() || MO.getReg() != PCPU::SR)
          continue;
        if (MO.isDef()) {
          isSafe = true;
          break;
...
bool PCPUInstrInfo::analyzeSelect(const MachineInstr &MI,
                                   SmallVectorImpl<MachineOperand> &Cond,
                                   unsigned &TrueOp, unsigned &FalseOp,
                                   bool &Optimizable) const {
  assert(MI.getOpcode() == PCPU::SELECT && "unknown select instruction");
  // Select operands:
  // 0: Def.
  // 1: True use.
  // 2: False use.
...
MachineInstr *
PCPUInstrInfo::optimizeSelect(MachineInstr &MI,
...
bool PCPUInstrInfo::analyzeBranch(MachineBasicBlock &MBB,
  ...
  while (Instruction != MBB.begin()) {
    ...
    if (Instruction->getOpcode() == PCPU::BT) {
      if (!AllowModify) {
...
bool PCPUInstrInfo::reverseBranchCondition(
  ...
  assert((Condition.size() == 1) &&
         "PCPU branch conditions should have one component.");
...
unsigned PCPUInstrInfo::insertBranch(MachineBasicBlock &MBB,
  ...
  if (Condition.empty()) {
    ...
    BuildMI(&MBB, DL, get(PCPU::BT)).addMBB(TrueBlock);
    return 1;
  }
  ...
  BuildMI(&MBB, DL, get(PCPU::BRCC)).addMBB(TrueBlock).addImm(ConditionalCode);
  ...
  BuildMI(&MBB, DL, get(PCPU::BT)).addMBB(FalseBlock);
  return 2;
}
...
unsigned PCPUInstrInfo::removeBranch(MachineBasicBlock &MBB,
  ...
  while (Instruction != MBB.begin()) {
    ...
    if (Instruction->isDebugInstr())
      continue;
    if (Instruction->getOpcode() != PCPU::BT &&
        Instruction->getOpcode() != PCPU::BRCC) {
      break;
    }
...
unsigned PCPUInstrInfo::isLoadFromStackSlot(const MachineInstr &MI,
                                             int &FrameIndex) const {
  if (MI.getOpcode() == PCPU::LDW_RI)
...
unsigned PCPUInstrInfo::isLoadFromStackSlotPostFE(const MachineInstr &MI,
                                                   int &FrameIndex) const {
  if (MI.getOpcode() == PCPU::LDW_RI) {
    unsigned Reg;
...
unsigned PCPUInstrInfo::isStoreToStackSlot(const MachineInstr &MI,
                                            int &FrameIndex) const {
  if (MI.getOpcode() == PCPU::SW_RI)
...
bool PCPUInstrInfo::getMemOperandWithOffsetWidth(
...
bool PCPUInstrInfo::getMemOperandsWithOffsetWidth(

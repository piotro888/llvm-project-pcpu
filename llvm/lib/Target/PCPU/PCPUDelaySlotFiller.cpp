//===-- PCPUDelaySlotFiller.cpp - PCPU delay slot filler ----------------===//
//
...
static cl::opt<bool>
    NopDelaySlotFiller("PCPU-nop-delay-filler", cl::init(false),
                       cl::desc("Fill PCPU delay slots with NOPs."),
                       cl::Hidden);
...
struct Filler : public MachineFunctionPass {
  ...
  StringRef getPassName() const override { return "PCPU Delay Slot Filler"; }

  bool runOnMachineBasicBlock(MachineBasicBlock &MBB);

  bool runOnMachineFunction(MachineFunction &MF) override {
    const PCPUSubtarget &Subtarget = MF.getSubtarget<PCPUSubtarget>();
...
// createPCPUDelaySlotFillerPass - Returns a pass that fills in delay
// slots in PCPU MachineFunctions
FunctionPass *
llvm::createPCPUDelaySlotFillerPass(const PCPUTargetMachine & /*tm*/) {
  return new Filler();
}
  ...
  for (MachineBasicBlock::instr_iterator I = MBB.instr_begin();
    ...
    if (I->getDesc().hasDelaySlot()) {
      ...
      if (I->getOpcode() == PCPU::RET) {
        ...
        assert(RI->getOpcode() == PCPU::LDW_RI && RI->getOperand(0).isReg() &&
               RI->getOperand(0).getReg() == PCPU::FP &&
               RI->getOperand(1).isReg() &&
               RI->getOperand(1).getReg() == PCPU::FP &&
               RI->getOperand(2).isImm() && RI->getOperand(2).getImm() == -8);
        ++RI;
        assert(RI->getOpcode() == PCPU::ADD_I_LO &&
               RI->getOperand(0).isReg() &&
               RI->getOperand(0).getReg() == PCPU::SP &&
               RI->getOperand(1).isReg() &&
               RI->getOperand(1).getReg() == PCPU::FP);
        ...
        if (!NopDelaySlotFiller && findDelayInstr(MBB, I, J)) {
          ...
          BuildMI(MBB, std::next(I), DebugLoc(), TII->get(PCPU::NOP));
        }
        ++FilledSlots;
      }

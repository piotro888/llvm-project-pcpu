...
static cl::opt<int> PCPULowerConstantMulThreshold(
    "PCPU-constant-mul-threshold", cl::Hidden,
...
//===----------------------------------------------------------------------===//
//                       PCPU Inline Assembly Support
//===----------------------------------------------------------------------===//

Register PCPUTargetLowering::getRegisterByName(
...
PCPUTargetLowering::getSingleConstraintMatchWeight(
...
void PCPUTargetLowering::LowerAsmOperandForConstraint(
...
SDValue PCPUTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  // PCPU target does not yet support tail call optimization.
  IsTailCall = false;
...
SDValue PCPUTargetLowering::LowerCCCArguments(
  ...
  // The PCPU ABI for returning structs by value requires that we copy
...
bool PCPUTargetLowering::CanLowerReturn(
...
SDValue PCPUTargetLowering::LowerCCCCallTo(
  ...
  uint8_t OpFlag = PCPUII::MO_NO_FLAG;
  if (G) {
...
static LPCC::CondCode IntCondCCodeToICC(SDValue CC, const SDLoc &DL,
  ...
  // and PCPU only supports integer comparisons, so only provide definitions
  // for them.
  switch (SetCCOpcode) {
  case ISD::SETEQ:
    return LPCC::ICC_EQ;
...
SDValue PCPUTargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue Flag =
      DAG.getNode(PCPUISD::SET_FLAG, DL, MVT::Glue, LHS, RHS, TargetCC);

  return DAG.getNode(PCPUISD::BR_CC, DL, Op.getValueType(), Chain, Dest,
                     TargetCC, Flag);
}

SDValue PCPUTargetLowering::LowerMUL(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (InstrRequired > PCPULowerConstantMulThreshold)
    return SDValue();
...
SDValue PCPUTargetLowering::LowerSETCC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(PCPUISD::SETCC, DL, Op.getValueType(), TargetCC, Flag);
}

SDValue PCPUTargetLowering::LowerSELECT_CC(SDValue Op,
                                            SelectionDAG &DAG) const {
  SDValue LHS = Op.getOperand(0);
  SDValue RHS = Op.getOperand(1);
  ...
  return DAG.getNode(PCPUISD::SELECT_CC, DL, VTs, TrueV, FalseV, TargetCC,
                     Flag);
}
...
SDValue PCPUTargetLowering::LowerDYNAMIC_STACKALLOC(SDValue Op,
                                                     SelectionDAG &DAG) const {
  SDValue Chain = Op.getOperand(0);
  ...
  // For PCPU, the outgoing memory arguments area should be on top of the
  ...
  SDValue ArgAdjust = DAG.getNode(PCPUISD::ADJDYNALLOC, DL, MVT::i32, Sub);
...
SDValue PCPUTargetLowering::LowerRETURNADDR(SDValue Op,
                                             SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  MachineFrameInfo &MFI = MF.getFrameInfo();
...
SDValue PCPUTargetLowering::LowerFRAMEADDR(SDValue Op,
                                            SelectionDAG &DAG) const {
  MachineFrameInfo &MFI = DAG.getMachineFunction().getFrameInfo();
  ...
  EVT VT = Op.getValueType();
  SDLoc DL(Op);
  SDValue FrameAddr = DAG.getCopyFromReg(DAG.getEntryNode(), DL, PCPU::FP, VT);
  unsigned Depth = cast<ConstantSDNode>(Op.getOperand(0))->getZExtValue();
  while (Depth--) {
...
const char *PCPUTargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch (Opcode) {
  case PCPUISD::ADJDYNALLOC:
    return "PCPUISD::ADJDYNALLOC";
  case PCPUISD::RET_FLAG:
    return "PCPUISD::RET_FLAG";
  case PCPUISD::CALL:
    return "PCPUISD::CALL";
  case PCPUISD::SELECT_CC:
    return "PCPUISD::SELECT_CC";
  case PCPUISD::SETCC:
    return "PCPUISD::SETCC";
  case PCPUISD::SUBBF:
    return "PCPUISD::SUBBF";
  case PCPUISD::SET_FLAG:
    return "PCPUISD::SET_FLAG";
  case PCPUISD::BR_CC:
  ...
  case PCPUISD::LO:
    return "PCPUISD::LO";
  case PCPUISD::SMALL:
    return "PCPUISD::SMALL";
  default:
    return nullptr;
  }
}

SDValue PCPUTargetLowering::LowerConstantPool(SDValue Op,
                                               SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ConstantPoolSDNode *N = cast<ConstantPoolSDNode>(Op);
  const Constant *C = N->getConstVal();
  const PCPUTargetObjectFile *TLOF =
      static_cast<const PCPUTargetObjectFile *>(
  ...
  if (getTargetMachine().getCodeModel() == CodeModel::Small ||
    ...
    SDValue Small = DAG.getTargetConstantPool(
        C, MVT::i32, N->getAlign(), N->getOffset(), PCPUII::MO_NO_FLAG);
    return DAG.getNode(ISD::OR, DL, MVT::i32,
                       DAG.getRegister(PCPU::R0, MVT::i32),
                       DAG.getNode(PCPUISD::SMALL, DL, MVT::i32, Small));
  } else {
    uint8_t OpFlagHi = PCPUII::MO_ABS_HI;
    uint8_t OpFlagLo = PCPUII::MO_ABS_LO;
    ...
    Hi = DAG.getNode(PCPUISD::HI, DL, MVT::i32, Hi);
    Lo = DAG.getNode(PCPUISD::LO, DL, MVT::i32, Lo);
...
SDValue PCPUTargetLowering::LowerGlobalAddress(SDValue Op,
  ...
  if (TLOF->isGlobalInSmallSection(GO, getTargetMachine())) {
    SDValue Small = DAG.getTargetGlobalAddress(
        GV, DL, getPointerTy(DAG.getDataLayout()), Offset, PCPUII::MO_NO_FLAG);
    return DAG.getNode(ISD::OR, DL, MVT::i32,
                       DAG.getRegister(PCPU::R0, MVT::i32),
                       DAG.getNode(PCPUISD::SMALL, DL, MVT::i32, Small));
  } else {
...
SDValue PCPUTargetLowering::LowerBlockAddress(SDValue Op,
                                               SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue PCPUTargetLowering::LowerSHL_PARTS(SDValue Op,
                                            SelectionDAG &DAG) const {
  EVT VT = Op.getValueType();
  ...
  // PCPU shifts of larger than register sizes are wrapped rather than
...
SDValue PCPUTargetLowering::LowerSRL_PARTS(SDValue Op,
                                            SelectionDAG &DAG) const {
  ...
  // Note: This takes advantage of PCPU's shift behavior to avoid needing to
...
SDValue PCPUTargetLowering::PerformDAGCombine(SDNode *N,
...
void PCPUTargetLowering::computeKnownBitsForTargetNode(

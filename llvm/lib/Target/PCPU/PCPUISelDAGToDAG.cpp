...
//===----------------------------------------------------------------------===//
// PCPUDAGToDAGISel - PCPU specific code to select PCPU machine
...
// Helper functions for ComplexPattern used on PCPUInstrInfo
// Used on PCPU Load/Store instructions.
bool PCPUDAGToDAGISel::selectAddrSls(SDValue Addr, SDValue &Offset) {
  ...
  if (Addr.getOpcode() == ISD::OR &&
      Addr.getOperand(1).getOpcode() == PCPUISD::SMALL) {
...
bool PCPUDAGToDAGISel::selectAddrRiSpls(SDValue Addr, SDValue &Base,
  ...
  if (ConstantSDNode *CN = dyn_cast<ConstantSDNode>(Addr)) {
    if (RiMode) {
      ...
      if (isInt<16>(CN->getSExtValue())) {
        ...
        Offset = CurDAG->getTargetConstant(Imm, DL, CN->getValueType(0));
        Base = CurDAG->getRegister(PCPU::R0, CN->getValueType(0));
...
bool PCPUDAGToDAGISel::selectAddrRi(SDValue Addr, SDValue &Base,
                                     SDValue &Offset, SDValue &AluOp) {
  return selectAddrRiSpls(Addr, Base, Offset, AluOp, /*RiMode=*/true);
}

bool PCPUDAGToDAGISel::selectAddrSpls(SDValue Addr, SDValue &Base,
                                       SDValue &Offset, SDValue &AluOp) {
  return selectAddrRiSpls(Addr, Base, Offset, AluOp, /*RiMode=*/false);
}

bool PCPUDAGToDAGISel::selectAddrRr(SDValue Addr, SDValue &R1, SDValue &R2,
                                     SDValue &AluOp) {
  // if Address is FI, get the TargetFrameIndex.
  ...
  ISD::NodeType AluOperator = static_cast<ISD::NodeType>(Addr.getOpcode());
  LPAC::AluCode AluCode = LPAC::isdToPCPUAluCode(AluOperator);
  if (AluCode != LPAC::UNKNOWN) {
    ...
    if (Addr.getOperand(0).getOpcode() == PCPUISD::HI ||
        Addr.getOperand(0).getOpcode() == PCPUISD::LO ||
        Addr.getOperand(0).getOpcode() == PCPUISD::SMALL ||
        Addr.getOperand(1).getOpcode() == PCPUISD::HI ||
        Addr.getOperand(1).getOpcode() == PCPUISD::LO ||
        Addr.getOperand(1).getOpcode() == PCPUISD::SMALL)
      return false;

...
class PCPUAsmParser : public MCTargetAsmParser {
  // Parse operands
  std::unique_ptr<PCPUOperand> parseRegister(bool RestoreOnFailure = false);

  std::unique_ptr<PCPUOperand> parseImmediate();

  std::unique_ptr<PCPUOperand> parseIdentifier();
...
// PCPUOperand - Instances of this class represented a parsed machine
// instruction
struct PCPUOperand : public MCParsedAsmOperand {
  ...
  explicit PCPUOperand(KindTy Kind) : Kind(Kind) {}
...
public:
  ...
  bool isHiImm16() {
    ...
    if (const PCPUMCExpr *SymbolRefExpr = dyn_cast<PCPUMCExpr>(Imm.Value))
      return SymbolRefExpr->getKind() == PCPUMCExpr::VK_PCPU_ABS_HI;

    // Binary expression
    if (const MCBinaryExpr *BinaryExpr = dyn_cast<MCBinaryExpr>(Imm.Value))
      if (const PCPUMCExpr *SymbolRefExpr =
              dyn_cast<PCPUMCExpr>(BinaryExpr->getLHS()))
        return SymbolRefExpr->getKind() == PCPUMCExpr::VK_PCPU_ABS_HI;
  ...
  bool isLoImm16() {
    ...
    // Symbolic reference expression
    if (const PCPUMCExpr *SymbolRefExpr = dyn_cast<PCPUMCExpr>(Imm.Value))
      return SymbolRefExpr->getKind() == PCPUMCExpr::VK_PCPU_ABS_LO;
  ...
  bool isLoImm21() {
    ...
    // Symbolic reference expression
    if (const PCPUMCExpr *SymbolRefExpr = dyn_cast<PCPUMCExpr>(Imm.Value))
      return SymbolRefExpr->getKind() == PCPUMCExpr::VK_PCPU_None;
  ...
  void addLoImm16Operands(MCInst &Inst, unsigned N) const {
    ...
    if (const MCConstantExpr *ConstExpr = dyn_cast<MCConstantExpr>(getImm()))
      Inst.addOperand(
          MCOperand::createImm(static_cast<int32_t>(ConstExpr->getValue())));
    else if (isa<PCPUMCExpr>(getImm())) {
#ifndef NDEBUG
      const PCPUMCExpr *SymbolRefExpr = dyn_cast<PCPUMCExpr>(getImm());
      assert(SymbolRefExpr &&
             SymbolRefExpr->getKind() == PCPUMCExpr::VK_PCPU_ABS_LO);
#endif
      Inst.addOperand(MCOperand::createExpr(getImm()));
    } else if (isa<MCBinaryExpr>(getImm())) {
#ifndef NDEBUG
      const MCBinaryExpr *BinaryExpr = dyn_cast<MCBinaryExpr>(getImm());
      assert(BinaryExpr && isa<PCPUMCExpr>(BinaryExpr->getLHS()) &&
             cast<PCPUMCExpr>(BinaryExpr->getLHS())->getKind() ==
                 PCPUMCExpr::VK_PCPU_ABS_LO);
#endif
      Inst.addOperand(MCOperand::createExpr(getImm()));
    } else
  ...
  void addHiImm16Operands(MCInst &Inst, unsigned N) const {
    ...
    if (const MCConstantExpr *ConstExpr = dyn_cast<MCConstantExpr>(getImm()))
      ...
      const PCPUMCExpr *SymbolRefExpr = dyn_cast<PCPUMCExpr>(getImm());
      assert(SymbolRefExpr &&
             SymbolRefExpr->getKind() == PCPUMCExpr::VK_PCPU_ABS_HI);
#endif
      Inst.addOperand(MCOperand::createExpr(getImm()));
    } else if (isa<MCBinaryExpr>(getImm())) {
#ifndef NDEBUG
  ...
  void addLoImm21Operands(MCInst &Inst, unsigned N) const {
    ...
    if (const MCConstantExpr *ConstExpr = dyn_cast<MCConstantExpr>(getImm()))
      ...
      const PCPUMCExpr *SymbolRefExpr = dyn_cast<PCPUMCExpr>(getImm());
      assert(SymbolRefExpr &&
             SymbolRefExpr->getKind() == PCPUMCExpr::VK_PCPU_None);
#endif
      Inst.addOperand(MCOperand::createExpr(getImm()));
  ...
  static std::unique_ptr<PCPUOperand> CreateToken(StringRef Str, SMLoc Start) {
  ...
  static std::unique_ptr<PCPUOperand> createReg(unsigned RegNum, SMLoc Start,
                                                 SMLoc End) {
  ...
  static std::unique_ptr<PCPUOperand> createImm(const MCExpr *Value,
  ...
  static std::unique_ptr<PCPUOperand>
  MorphToMemImm(std::unique_ptr<PCPUOperand> Op) {
  ...
  static std::unique_ptr<PCPUOperand>
  MorphToMemRegReg(unsigned BaseReg, std::unique_ptr<PCPUOperand> Op,
                   unsigned AluOp) {
  ...
  static std::unique_ptr<PCPUOperand>
  MorphToMemRegImm(unsigned BaseReg, std::unique_ptr<PCPUOperand> Op,
                   unsigned AluOp) {
    const MCExpr *Imm = Op->getImm();
...
std::unique_ptr<PCPUOperand>
PCPUAsmParser::parseRegister(bool RestoreOnFailure) {
  ...
  if (Lexer.getKind() == AsmToken::Identifier) {
    ...
    return PCPUOperand::createReg(RegNum, Start, End);
  }
  if (PercentTok && RestoreOnFailure)
    Lexer.UnLex(*PercentTok);
  return nullptr;
...
bool PCPUAsmParser::parseRegister(MCRegister &RegNum, SMLoc &StartLoc,
                                   SMLoc &EndLoc) {
  const AsmToken &Tok = getParser().getTok();
  StartLoc = Tok.getLoc();
  EndLoc = Tok.getEndLoc();
  std::unique_ptr<PCPUOperand> Op = parseRegister(/*RestoreOnFailure=*/false);
  if (Op != nullptr)
    RegNum = Op->getReg();
  return (Op == nullptr);
}

OperandMatchResultTy PCPUAsmParser::tryParseRegister(MCRegister &RegNum,
                                                      SMLoc &StartLoc,
                                                      SMLoc &EndLoc) {
  const AsmToken &Tok = getParser().getTok();
  StartLoc = Tok.getLoc();
  EndLoc = Tok.getEndLoc();
  std::unique_ptr<PCPUOperand> Op = parseRegister(/*RestoreOnFailure=*/true);
  if (Op == nullptr)
...
std::unique_ptr<PCPUOperand> PCPUAsmParser::parseIdentifier() {
  SMLoc Start = Parser.getTok().getLoc();
  SMLoc End = SMLoc::getFromPointer(Parser.getTok().getLoc().getPointer() - 1);
  const MCExpr *Res, *RHS = nullptr;
  PCPUMCExpr::VariantKind Kind = PCPUMCExpr::VK_PCPU_None;
  ...
  // identifier.
  if (Kind != PCPUMCExpr::VK_PCPU_None) {
  ...
  Res = PCPUMCExpr::create(Kind, Expr, getContext());

  // Nest if this was an addition
  if (RHS)
    Res = MCBinaryExpr::createAdd(Res, RHS, getContext());

  return PCPUOperand::createImm(Res, Start, End);
}

std::unique_ptr<PCPUOperand> PCPUAsmParser::parseImmediate() {
  SMLoc Start = Parser.getTok().getLoc();
  SMLoc End = SMLoc::getFromPointer(Parser.getTok().getLoc().getPointer() - 1);
  ...
  case AsmToken::Dot:
    if (!Parser.parseExpression(ExprVal))
      return PCPUOperand::createImm(ExprVal, Start, End);
    [[fallthrough]];
  default:
    return nullptr;
  }
...
unsigned PCPUAsmParser::parseAluOperator(bool PreOp, bool PostOp) {
  StringRef IdString;
  Parser.parseIdentifier(IdString);
  unsigned AluCode = LPAC::stringToPCPUAluCode(IdString);
...
bool PCPUAsmParser::parsePrePost(StringRef Type, int *OffsetValue) {
...
bool shouldBeSls(const PCPUOperand &Op) {
  ...
  if (const PCPUMCExpr *SymbolRefExpr = dyn_cast<PCPUMCExpr>(Op.getImm()))
    return SymbolRefExpr->getKind() == PCPUMCExpr::VK_PCPU_None;
  ...
  if (const MCBinaryExpr *BinaryExpr = dyn_cast<MCBinaryExpr>(Op.getImm())) {
    const PCPUMCExpr *LHSSymbolRefExpr =
        dyn_cast<PCPUMCExpr>(BinaryExpr->getLHS());
    return (LHSSymbolRefExpr &&
            LHSSymbolRefExpr->getKind() == PCPUMCExpr::VK_PCPU_None);
  }
  return false;
}
...
PCPUAsmParser::parseMemoryOperand(OperandVector &Operands) {
  ...
  if (Operands[0]->isToken())
    Type = static_cast<PCPUOperand *>(Operands[0].get())->getToken();
  ...
  std::unique_ptr<PCPUOperand> Op = parseRegister();
  if (!Op)
    Op = parseImmediate();
  ...
  Parser.Lex(); // Eat the '['.
  std::unique_ptr<PCPUOperand> Offset = nullptr;
  if (Op)
    Offset.swap(Op);
  ...
  if (!Op) {
    if (!Offset) {
      if ((Op = parseImmediate()) && Lexer.is(AsmToken::RBrac)) {
        ...
        // SLS, the rest as RM.
        if (shouldBeSls(*Op)) {
          Operands.push_back(PCPUOperand::MorphToMemImm(std::move(Op)));
        } else {
          ...
          Operands.push_back(PCPUOperand::MorphToMemRegImm(
              PCPU::R0, std::move(Op), LPAC::ADD));
        }
        return MatchOperand_Success;
      }
    }
  ...
  if (Lexer.is(AsmToken::RBrac)) {
    ...
    if (!Offset) {
      ...
      Offset = PCPUOperand::createImm(OffsetConstExpr, Start, End);
    }
  } else {
  ...
  Operands.push_back(
      Offset->isImm()
          ? PCPUOperand::MorphToMemRegImm(BaseReg, std::move(Offset), AluOp)
          : PCPUOperand::MorphToMemRegReg(BaseReg, std::move(Offset), AluOp));
...
OperandMatchResultTy
PCPUAsmParser::parseOperand(OperandVector *Operands, StringRef Mnemonic) {
...
StringRef PCPUAsmParser::splitMnemonic(StringRef Name, SMLoc NameLoc,
  ...
  if (Mnemonic[0] == 'b' ||
    ...
    LPCC::CondCode CondCode =
        LPCC::suffixToPCPUCondCode(Mnemonic.substr(1, Next));
    if (CondCode != LPCC::UNKNOWN) {
      ...
      if (IsBRR) {
        Operands->push_back(PCPUOperand::CreateToken(".r", NameLoc));
      }
      return Mnemonic;
    }
  }
  ...
  if (Mnemonic.startswith("sel") ||
    ...
    LPCC::CondCode CondCode = LPCC::suffixToPCPUCondCode(Mnemonic);
    if (CondCode != LPCC::UNKNOWN) {
...
static bool IsMemoryAssignmentError(const OperandVector &Operands) {
  ...
  if (PCPUOperand *PossibleAluOp =
          static_cast<PCPUOperand *>(Operands[PossibleAluOpIdx].get()))
...
static bool IsRegister(const MCParsedAsmOperand &op) {
  return static_cast<const PCPUOperand &>(op).isReg();
}
...
static bool MaybePredicatedInst(const OperandVector &Operands) {
  ...
  return StringSwitch<bool>(
             static_cast<const PCPUOperand &>(*Operands[0]).getToken())
...
bool PCPUAsmParser::ParseInstruction(ParseInstructionInfo & /*Info*/,
  ...
  if (Lexer.is(AsmToken::EndOfStatement) && Name == "st" &&
    ...
    Operands.insert(Operands.begin(), PCPUOperand::CreateToken("s", NameLoc));
    Operands.insert(Operands.begin() + 1,
                    PCPUOperand::createImm(
  ...
  if (Lexer.is(AsmToken::EndOfStatement) && Name.startswith("bt") &&
    ...
    Operands.insert(Operands.begin(), PCPUOperand::CreateToken("bt", NameLoc));
  }
